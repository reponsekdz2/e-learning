...
const SubjectSelectionScreen: React.FC<SubjectSelectionScreenProps> = ({ subjects, onSelectSubject, subjectFilter }) => {
  const filteredSubjects = subjects.filter(subject => {
    if (subjectFilter === 'All') return true;
    return subject.category === subjectFilter;
  });
    ...
    <div className="text-center">
      ...
      {filteredSubjects.length > 0 ? (
        ...
        <div className="bg-gray-800 p-8 rounded-lg max-w-md mx-auto">
            <p className="text-xl text-gray-300">No subjects found in the "{subjectFilter}" category.</p>
            <p className="text-gray-400 mt-2">Try selecting another filter from the header.</p>
        </div>
      )}
    </div>
  );
